...
Dashboard ดึงข้อมูลจาก Supabase ใหม่อัตโนมัติทุก 15 นาทีขณะเปิดหน้าอยู่ และสามารถกด `รีเฟรชข้อมูล` เพื่ออัปเดตทันทีได้

Supabase Edge Function `sync-google-sheet` ถูกเรียกด้วย Database Cron ทุก 15 นาที เพื่ออ่าน Target เดือนล่าสุดและยอดขาย MTD ช่วงวันเดียวกันของเดือนปัจจุบัน เดือนก่อน และปีก่อนจาก Google Sheet แล้วแทนที่ snapshot ในฐานข้อมูลแบบ transaction เดียว หากการอ่านหรือ validation ล้มเหลว ระบบจะเก็บ snapshot เดิมไว้
